...
class Designation extends CActiveRecord
{
	...
	{
            if($status=='all'){
                $model = Designation::model()->findAll();
            }else if($status=='active'){
                 $model = Designation::model()->findAll('status=:status',array(':status'=>1));
            }else if($status=='inactive'){
                 $model = Designation::model()->findAll('status=:status',array(':status'=>0));
            }else{
